...
- (NSData *)yy_aes256EncryptWithKey:(NSString *)key iv:(NSString *)iv {
    ...
    // iv为nil，认为使用ECB模式加解密
    CCOptions ccOptionsMode = kCCOptionPKCS7Padding;
    if (iv.length == 0) {
        ccOptionsMode |= kCCOptionECBMode;
    }
...
- (NSData *)yy_3DESEncryptWithKey:(NSString *)key iv:(NSString *)iv {
    CCCryptorStatus status = kCCSuccess;
    NSData *result = [self yy_dataEncryptedUsingAlgorithm:kCCAlgorithm3DES key:key initializationVector:iv options:kCCOptionPKCS7Padding error:&status];
    return result;
}

- (NSData *)yy_3DESDecryptWithkey:(NSString *)key iv:(NSString *)iv {
    CCCryptorStatus status = kCCSuccess;
    NSData *result = [self yy_decryptedDataUsingAlgorithm:kCCAlgorithm3DES key:key initializationVector:iv options:kCCOptionPKCS7Padding error:&status];
    return result;
}

// Define the buffer size
#define YYCATEGORIESLIB_BUFFER_SIZE 1024

- (NSData *)yy_gzipCompress {
    // Allocate memory for the compressed data
    NSMutableData* compressedData = [NSMutableData dataWithLength:YYCATEGORIESLIB_BUFFER_SIZE];
    ...
    // Compress the data
    int result = deflateInit2(&stream, Z_DEFAULT_COMPRESSION, Z_DEFLATED, 31, 8, Z_DEFAULT_STRATEGY);
    if (result == Z_OK) {
        do {
...
- (NSData *)yy_gzipDecompress {
    ...
    NSMutableData* decompressedData = [NSMutableData dataWithLength:YYCATEGORIESLIB_BUFFER_SIZE];
    // Initialize the zlib stream
    z_stream stream;
    stream.zalloc = Z_NULL;
    stream.zfree = Z_NULL;
    ...
    if (result == Z_OK) {
        ...
        inflateEnd(&stream);
        if (result == Z_STREAM_END) {
            decompressedData.length = stream.total_out;
...
@implementation NSData (YYLowLevelCommonCryptor)

- (NSData *)yy_runCryptor: (CCCryptorRef) cryptor result: (CCCryptorStatus *) status
{
                                   ...
                                   error: (CCCryptorStatus *) error
{
    ...
    NSData * result = [self yy_runCryptor: cryptor result: &status];
