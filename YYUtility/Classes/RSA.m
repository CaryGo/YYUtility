...
+ (NSData *)encryptData:(NSData *)data publicKeyWithContentsOfFile:(NSString *)path{
    ...
    return [self encryptData:data publicKeyRef:[self getPublicKeyRefWithContentsOfFile:path]];
}
...
+ (NSData *)encryptData:(NSData *)data publicKeyRef:(SecKeyRef)publicKeyRef{
    if (!data) {
        return nil;
    }
    if(!publicKeyRef){
...
+ (NSData *)decryptData:(NSData *)data privateKeyWithContentsOfFile:(NSString *)path password:(NSString *)password{
    ...
    return [self decryptData:data privateKeyRef:[self getPrivateKeyRefWithContentsOfFile:path password:password]];
}
...
+ (NSData *)decryptData:(NSData *)data privateKeyRef:(SecKeyRef)privKeyRef{
    if (!privKeyRef) {
        return nil;
    }
    return [self decryptData:data withKeyRef:privKeyRef];
}
...
+ (SecKeyRef)addPublicKey:(NSString *)key{
    ...
    NSData *data = rsa_base64_decode(key);
...
// MARK: - Signature & Verify Signature
...
+ (NSData *)signatureData:(NSData *)data privateKey:(NSString *)privKey {
    ...
    return [RSASignature signData:data privateKey:keyRef];
}
...
+ (BOOL)verifyData:(NSData *)data signature:(NSData *)signatureData publicKey:(NSString *)pubKey {
    ...
    return [RSASignature verifyData:data signature:signatureData publicKey:keyRef];
}
